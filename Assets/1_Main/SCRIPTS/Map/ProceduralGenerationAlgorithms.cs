...
public static class ProceduralGenerationAlgorithms
{
    ...
    public static List<Vector2Int> RandomWalkCorridor(Vector2Int startposition,int corridorlength,int corridorWidth) {
        ...
        var randomdirection = Direction2D.GetrandomDirection();
        var curposition = startposition;
        for(int i = 0; i < corridorlength;i++) {
            for(int j = -corridorWidth/2 ; j < corridorWidth/2 + corridorWidth % 2 ; j++) {
                if(j != 0)
                {
                    Vector2Int dir = Vector2Int.right;
                    if(randomdirection == Vector2Int.right || randomdirection == Vector2Int.left) {
                        dir = Vector2Int.down;
                    }
                    else if(randomdirection == Vector2Int.up || randomdirection == Vector2Int.down) {
                        dir = Vector2Int.right;
                    }
                    corridor.Add(curposition + dir * j);
                }
            }
            corridor.Add(curposition);
            curposition += randomdirection;
        }
        return corridor;
    }
